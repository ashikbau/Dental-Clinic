...
const Login = () => {
    ...
    const { signIn,providerGoogleLogin,resetPassword ,loading,setLoading,providerTwitterLogin  } = useContext(AuthContext);
  ...
  const handleReset = () => {
    ...
    if(!loginError){
      ...
      resetPassword(loginError)
        .then(() => {
          console.log("email from resest",loginUserEmail)
    ...
    return (
        ...
        //                 <input type="text" name='email' onBlur={event => setLoginUserEmail(event.target.value)}
        //                     {...register("email", {
        ...
        //          <small className="label-text">Forget Password? <button onClick={handleReset} className='btn btn-outline w-full'>ResetPassword</button></small>
